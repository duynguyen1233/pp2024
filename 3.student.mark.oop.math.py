...
class StudentManagementSystem:
    ...
    def input_students(self):
        ...
        for _ in range(num_students):
            ...
            students_data.append([student_id, name, dob])
        self.students = np.array(students_data)
    ...
    def input_courses(self):
        ...
        for _ in range(num_courses):
            ...
            courses_data.append([course_id, name])
        self.courses = np.array(courses_data)

    def input_student_marks(self):
        course_id = input("Enter course ID: ")
        for i, student in enumerate(self.students):
            mark = float(input(f"Enter the mark for {student[1]} in {course_id}: "))
            self.students[i] = np.append(student, mark)

    def list_students(self):
        if not self.students.size:
    ...
    def show_student_marks(self):
        ...
        for student in self.students:
            if student.shape[0] > 3:
                mark_index = np.where(student[3:] != "")[0]
                if course_id in self.courses[mark_index]:
                    print(f"{student[1]}: {student[mark_index]}")
    ...
    def main(self):
        while True:
            ...
            if option == 0:
                ...
                if not self.students.size or not self.courses.size:
